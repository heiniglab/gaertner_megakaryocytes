...
print("Heatmap of cluster defining genes")

print(DotPlot(MKP3HTO, features = unique(top10$gene))+ RotatedAxis())
```

# gene set enrichment analysis using enrichR for the top 50 marker genes for each cluster
...
### defining genes per cluster using a cluster-specific barplot 
```{r expression, echo=TRUE}
print("the expression fold chnages of top 10 genes for each cluster vs every other cluster")
...
### getting proportion of cells per cluster for each condition
...
## response to type I interferon genes
...
### bulk seq DE genes- projecting the genes onto the single cell data per category
...
# projecting the common bulk sequencing genes between genes upregulated between (PD vs CNTRL) and downregulated between (PDC+PD VS CNTRL) onto the single-cell clusters
...
## This block of code finds the differentially expressed genes for each cluster between the three conditions (Bl6 vs PD, PD vs pDC-PD, and Bl6 vs pDC-PD). The DE genes are selected based on the threshold of avg_log2FC > 0.25 and p_val_adj < 0.05. The DE genes are then plotted onto a volcano plot using the package EnhancedVolcano.
...
# scatterplots to plot the common genes between single-cell clusters (MK-MEP cycling and MK-MEP) and bulk-seq PD vs CNTRL genes
...
```{r lib}
## libraries and reading in input
